...
from palabra_ai.benchmark.report import (
    BENCHMARK_ALLOWED_MESSAGE_TYPES,
    INPUT_CHUNK_DURATION_S,
    Report,
    Sentence,
    Tid,
    format_report,
...
from palabra_ai.message import Dbg, IoEvent, Message
from palabra_ai.model import IoData, LogData, RunResult
...
def test_benchmark_config_to_dict():
    ...
    test_config = {
        ...
        "pipeline": {
            ...
            "translations": [{"target_language": "es"}],
            "translation_queue_configs": {
                "global": {"auto_tempo": False, "min_tempo": 1.5, "max_tempo": 1.8}
            },
        },
    }
                        ...
                        with patch("palabra_ai.benchmark.__main__.tqdm"):
                            try:
                                ...
                                raise AssertionError(
                                    "main() should have raised RuntimeError"
                                )
...
def test_benchmark_saves_error_to_file_with_out():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("palabra_ai.benchmark.__main__.PalabraAI") as mock_palabra_class:
            ...
            with patch(
                "sys.argv",
                ["benchmark", "dummy.wav", "en", "es", "--out", str(output_dir)],
            ):
                with patch("palabra_ai.benchmark.__main__.Path") as mock_path_class:
                    ...
                    with patch("av.open"):
                        with patch("palabra_ai.benchmark.__main__.FileReader"):
                            with patch("palabra_ai.benchmark.__main__.tqdm"):
                                with patch(
                                    "palabra_ai.benchmark.__main__.get_system_info",
                                    ...
                                    try:
                                        ...
                                        error_files = list(
                                            output_dir.glob("*_bench_error.txt")
                                        )
                                        assert len(error_files) == 1, (
                                            f"Expected 1 error file, found {len(error_files)}"
                                        )
                                        ...
                                        assert (
                                            "Traceback" in error_content
                                            or "traceback" in error_content
                                        )
                        ...
                        with patch("palabra_ai.benchmark.__main__.FileReader"):
                            with patch("palabra_ai.benchmark.__main__.tqdm"):
                                with patch(
                                    ...
                                    with patch(
                                        "palabra_ai.benchmark.__main__.Report.parse",
                                        return_value=(
                                            MagicMock(),
                                            MagicMock(),
                                            MagicMock(),
                                        ...
                                        with patch(
                                            "palabra_ai.benchmark.__main__.format_report",
                                            return_value="Test report",
                                        ):
                                            with patch(
                                                "palabra_ai.benchmark.__main__.save_wav"
                                            ):
                                                try:
                                                    main()
                                                except Exception:
                                                ...
                                                assert len(sysinfo_files) >= 1, (
                                                    f"Expected at least 1 sysinfo file, found {len(sysinfo_files)}"
                                                )
...
def test_benchmark_handles_cancelled_error():
    ...
    failed_result = RunResult(
        ok=False, exc=cancelled_exc, io_data=None, log_data=log_data
    )
                            ...
                            with patch("palabra_ai.benchmark.__main__.tqdm"):
                                with patch(
                                    ...
                                    try:
                                        ...
                                        runresult_files = list(
                                            output_dir.glob(
                                                "*_bench_runresult_debug.json"
                                            )
                                        )
                                        assert len(runresult_files) >= 1, (
                                            f"Expected RunResult debug file, found {len(runresult_files)}"
                                        )
                                        ...
                                        assert len(error_files) >= 1, (
                                            f"Expected error file, found {len(error_files)}"
                                        )

                                        # Check that output mentions "cascade cancellation"
                                        output = captured_output.getvalue()
                                        assert "cascade cancellation" in output, (
                                            "Should mention cascade cancellation"
                                        )

                                        # Check that ALL logs were printed (not just last 100)
                                        assert (
                                            f"Full logs (all {len(log_entries)} entries)"
                                            in output
                                        )
...
def test_sysinfo_contains_command():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch(
            "sys.argv", ["benchmark", "test.wav", "en", "es", "--out", str(output_dir)]
        ):
            with patch("palabra_ai.benchmark.__main__.Path") as mock_path_class:
                ...
                with patch("av.open"):
                    ...
                    assert len(sysinfo_files) >= 1, (
                        f"Expected sysinfo file, found {len(sysinfo_files)}"
                    )
...
def test_manager_has_graceful_completion_flag():
    ...
    assert "_graceful_completion" in field_names, (
        "Manager should have _graceful_completion field"
    )
...
def test_benchmark_parse_handles_part_suffixes():
    ...
    events = [
        ...
        make_event(
            20, "sentence_2_part_0", "partial_transcription", base_ts + 1.0, "World"
        ),
        make_event(
            21, "sentence_2_part_0", "validated_transcription", base_ts + 1.1, "World"
        ),
        make_event(
            22, "sentence_2_part_0", "translated_transcription", base_ts + 1.2, "Mundo"
        ),
    ...
    assert len(report.sentences) == 4, (
        f"Expected 4 sentences, got {len(report.sentences)}"
    )
    ...
    assert s2p0.metric_partial is not None, (
        "sentence_2_part_0 should have metric_partial"
    )
    assert s2p0.metric_validated is not None, (
        "sentence_2_part_0 should have metric_validated"
    )
    assert s2p0.metric_translated is not None, (
        "sentence_2_part_0 should have metric_translated"
    )
    ...
    assert s2p1.metric_partial is None, (
        "sentence_2_part_1 should NOT have metric_partial"
    )
    assert s2p1.metric_validated is None, (
        "sentence_2_part_1 should NOT have metric_validated"
    )
    assert s2p1.metric_translated is None, (
        "sentence_2_part_1 should NOT have metric_translated"
    )
    ...
    assert s2p2.metric_partial is None, (
        "sentence_2_part_2 should NOT have metric_partial"
    )
    assert s2p2.metric_validated is None, (
        "sentence_2_part_2 should NOT have metric_validated"
    )
    assert s2p2.metric_translated is None, (
        "sentence_2_part_2 should NOT have metric_translated"
    )
    ...
    partial_count = len(
        [s for s in report.sentences.values() if s.metric_partial is not None]
    )
    ...
    assert s2p0.local_start_ts == s2p1.local_start_ts, (
        "sentence_2_part_1 should use parent timestamp"
    )
    assert s2p0.local_start_ts == s2p2.local_start_ts, (
        "sentence_2_part_2 should use parent timestamp"
    )
    ...
    report_text = format_report(
        report, io_data, "en", "es", "test.wav", "out.wav", config
    )
...
def test_benchmark_parse_handles_partial_extra_parts():
    ...
    events = [
        ...
        # Only validated
        make_event(
            20, "s1_part_1", "validated_transcription", base_ts + 1.0, "Only validated"
        ),
        # Only translated
        make_event(
            30, "s1_part_2", "translated_transcription", base_ts + 1.5, "Solo traducido"
        ),
    ]
    ...
    assert len(report.sentences) == 3, (
        f"Expected 3 sentences, got {len(report.sentences)}"
    )
...
def test_benchmark_parse_handles_orphan_extra_parts():
    ...
    events = [
        ...
        make_event(
            10, "orphan_part_1", "validated_transcription", base_ts + 0.5, "Orphan"
        ),
        make_event(
            11, "orphan_part_1", "translated_transcription", base_ts + 0.6, "Huerfano"
        ),
    ]
...
def test_benchmark_handles_sentence_splitter_case():
    ...
    assert "base_sentence" in parent_timestamps, (
        "base_sentence should be in parent_timestamps from _part_0"
    )
...
def test_benchmark_handles_partial_translated_transcription():
    ...
    events = [
        make_event(0, None, "input_audio_data", base_ts),
        make_event(
            10, base_tid, "partial_transcription", base_ts + 0.5, "validated text"
        ),
        make_event(
            11,
            base_tid,
        ...
        make_event(
            12, base_tid, "validated_transcription", base_ts + 1.5, "validated text"
        ),
        make_event(
            13, base_tid, "translated_transcription", base_ts + 2.0, "translated text"
        ),
    ...
    report, _, _ = Report.parse(
        io_data, translated_metric_key="partial_translated_transcription"
    )
    ...
    assert (
        sentence.translated_text == "partial translated text"
    )  # Should use partial translated
...
def test_benchmark_handles_missing_partial_transcription():
    ...
    events = [
        make_event(0, None, "input_audio_data", base_ts),
        make_event(
            10, base_tid, "validated_transcription", base_ts + 0.5, "validated text"
        ),
        make_event(
            11, base_tid, "translated_transcription", base_ts + 1.0, "translated text"
        ),
...
def test_benchmark_forces_100ms_chunks_with_config():
    ...
    try:
        ...
        assert config.mode.input_chunk_duration_ms == 320, (
            "Config from JSON should default to 320ms (this is the problem)"
        )
        ...
        config = Config.from_json(config_path.read_text())
        assert config.mode.input_chunk_duration_ms == 320, (
            "JSON config overwrites with 320ms default"
        )
        ...
        assert config.mode.input_chunk_duration_ms == 100, (
            "After fix: benchmark should force 100ms chunks"
        )
        ...
        with patch("sys.argv", ["rewind", str(result_file), "--out", str(output_dir)]):
            with patch("palabra_ai.benchmark.rewind.load_run_result") as mock_load:
                ...
                with patch("palabra_ai.benchmark.rewind.Report.parse") as mock_parse:
                    ...
                    with patch(
                        "palabra_ai.benchmark.rewind.format_report"
                    ) as mock_format:
                        mock_format.return_value = "Test report content"

                        with patch(
                            "palabra_ai.benchmark.rewind.Config.from_dict"
                        ) as mock_config:
                            ...
                            with patch(
                                "palabra_ai.benchmark.rewind.save_benchmark_files"
                            ) as mock_save:
                                # Run rewind
                                rewind_main()
...
def test_mixed_list_of_dicts():
    data = [{"foo": {"bar": False}}, {"bar": {"baz": 10}}, {"baz": {"xxx": {}}}]
...
def test_mixed_dict_of_lists_and_scalars():
    data = {"foo": {"bar": [{"baz": []}]}, "xxx": None, "zzz": "hello"}
    got = flatten_container_to_paths(data)
    want = [
        ("foo.bar.0.baz", []),
        ("xxx", None),
...
def test_tts_buffer_breakdown_in_report():
    ...
    assert len(report.tts_buffer_events) == 3, (
        f"Expected 3 tts_buffer_events, got {len(report.tts_buffer_events)}"
    )
    ...
    assert "TTS BUFFER BREAKDOWN" in report_text, (
        "Report should contain TTS BUFFER BREAKDOWN section"
    )
...
def test_tts_buffer_breakdown_not_shown_when_empty():
    ...
    assert "TTS BUFFER BREAKDOWN" not in report_text, (
        "Report should NOT contain TTS BUFFER BREAKDOWN when no events"
    )
